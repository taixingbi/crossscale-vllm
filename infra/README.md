...
Applying creates billable resources: EKS, CPU instances, NAT, storage and subsequently GPU instances. The single NAT gateway is an experiment cost tradeoff, not a high-availability design. Local state is the default; keep it secure and available for teardown. For team use, configure a separate encrypted remote backend with locking for each root before applying. State, local variable files and saved plans are ignored; commit both provider lock files.
