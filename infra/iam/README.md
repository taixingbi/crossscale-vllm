...
The `crossscale-discovery.json` policy is read-only environment discovery restricted to us-east-1, plus reading its own IAM role and public EKS 1.34 AMI metadata. The separately authorized `crossscale-provision` policy now permits the reviewed cluster deployment. See [deployment scope and state](PROVISIONING.md). The deployment scope is us-east-1 and the dedicated CrossScale cluster/state resources. Do not attach the Bedrock role's unrelated account-creation and application permissions.
