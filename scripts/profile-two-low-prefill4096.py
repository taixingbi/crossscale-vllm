"""Separate two-GPU capacity profile at 4096 batch tokens."""
import copy
import asyncio
import fcntl
import json
...
with (root/'suite.lock').open('a') as lock:
    ...
    dest = root/'profile-two-low-prefill4096-control'
    ...
    try:
        ...
        study.warmup()
        result = study.sweep('profile-two-low-prefill4096', 2, [.005,.008,.01], duration=3600)
        save(dest/'complete.json',dict(completed_unix_s=time.time(),profile=result))
        study.event('prefill4096-two-low-profile-complete',sustainable_rps=result['sustainable_rps'])
    finally:
