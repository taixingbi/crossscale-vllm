"""Replay two-GPU profile B-tail misses serially on one 4096-token replica."""
import copy
import asyncio
import fcntl
import json
...
with (root/'suite.lock').open('a') as lock:
    ...
    dest = root/'diagnosis-two-profile-tail4096'
    ...
    try:
        ...
        study.warmup()
        config = json.loads((root/'profile-two-prefill4096/rps-0.013-seed-17/run/config.json').read_text())
        config.update(duration_s=2, drain_s=180)
        cases = []
        for seed, rid in ((17,0),(19,7)):
            source = root/f'profile-two-prefill4096/rps-0.013-seed-{seed}/run/requests.jsonl'
        ...
        cases += [dict(tenant='B',input_tokens=3072,output_tokens=256,id=0)]
