...
with (root/'suite.lock').open('a') as lock:
    ...
    dest = root/'profile-two-prefill4096-control'
    ...
    try:
        ...
        study.warmup()
        result = study.sweep('profile-two-prefill4096', 2, [.013,.02,.025], duration=1800)
        save(dest/'complete.json',dict(completed_unix_s=time.time(),profile=result))
        study.event('prefill4096-two-profile-complete',sustainable_rps=result['sustainable_rps'])
    finally:
