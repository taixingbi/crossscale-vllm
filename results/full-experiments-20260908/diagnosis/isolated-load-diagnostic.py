...
with (root/'suite.lock').open('a') as lock:
    ...
    study=Study(root)
    dest=root/'diagnosis-isolated-load'
    ...
    with Observer(study.k,dest):
        ...
        study.event('isolated-load-diagnostic-start',source=str(source),output=str(dest))
    ...
    study.event('isolated-load-diagnostic-complete',**result)
