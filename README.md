...
1. Prepare an EKS cluster with GPU device plugin, Karpenter NodePool/EC2NodeClass, KEDA and Prometheus. Fill in image/model placeholders in `deploy/vllm.yaml`; use a dedicated namespace. This repository does not create cloud infrastructure. Configure Prometheus discovery of vLLM and the gateway explicitly; annotations alone do not guarantee scraping.
